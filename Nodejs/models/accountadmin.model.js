...
Account.getAll = async function (req, callback) {
    ...
    sql += " order by account_id ASC LIMIT " + _start + "," + _limit;
...
Account.store = function(req,myFun){
    // console.log(req);
    let sql = `INSERT INTO account (account_name, email,password,role,created_at,phone_number,status, address,) VALUES ("${req.account_name}", "${req.email}","admin",NOW(),"${req.phone_number}","1", "${req.address})"`;
    console.log(sql);
